...
> 状态：**调查完成（2026-08-17）**——读取时机已定、策略已选（S2+S3），待实现清理逻辑 | 创建：2026-08-17
...
**S2 + S3（worker 退出时删 + session 删除时清兜底），否决 S1（spawn 后立即删）。**
...
2. ~~采用 S1（激进）/ S2+S3（推荐）/ S3（保守）？~~ → **已答（四）：S2+S3**
3. 失败时是否回退重写文件（重试逻辑）？

## 七、任务拆解（若立项通过）

- [x] 实验 A/B/C（mcp_args 写文件 + spawn + 删 + handoff 验证）— 已完成 2026-08-17，见四
- [ ] 按结果选择策略，实现清理逻辑（worker.py kill/退出路径 + session 删除钩子）— 策略已选 S2+S3

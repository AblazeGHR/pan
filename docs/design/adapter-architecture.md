...
|---|---|---|---|---|---|
| **执行模式** | stream 长驻（原生 stdin/stdout）+ one-shot MCP（`base_args_stream`，worker `_consumer_oneshot`） | wrapper 长驻（wrapper.py 内逐条 `kimi -p`） | wrapper 长驻（wrapper.py 内逐条 `opencode run`） | stream + oneshot（`--input-format stream-json`；`outputMode=oneshot` 时短进程） | stream 长驻（wrapper.py → 原生 `codex app-server --stdio`，`execution_modes=["stream"]`） |
| **事件协议** | stdin JSONL `{"type":"user","message":...}`；stdout stream-json（system.init/assistant/result/error） | wrapper stdin JSONL `{"text":text}`；`kimi -p --output-format stream-json`（meta/assistant/result + content.part，wrapper 转发/合成 result） | wrapper stdin JSONL `{"text":text}`；`opencode run --format json`（step_start/text/tool_use/step_finish/error，wrapper 合成 result） | stdin/stdout stream-json（system/assistant/result/error；`--include-partial-messages` 的 `stream_event` 归一化为 Pan 增量事件） | wrapper stdin JSONL → app-server JSON-RPC；原生 agent/reasoning/tool delta 转译为 Pan worker.stream，turn/completed 合成 result |

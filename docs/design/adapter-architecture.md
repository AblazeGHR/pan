...
|---|---|---|---|---|---|
| **执行模式** | stream 长驻（原生 stdin/stdout）+ one-shot MCP（`base_args_stream`，worker `_consumer_mcp`） | wrapper 长驻（wrapper.py 内逐条 `kimi -p`） | wrapper 长驻（wrapper.py 内逐条 `opencode run`） | one-shot（`execution_modes=["oneshot"]`，`claude -p` stream-json） | stream 长驻（wrapper.py → 原生 `codex app-server --stdio`，`execution_modes=["stream"]`） |

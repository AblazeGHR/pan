...
- 整体状态：批次 A、D 已由用户确认验收；`T-048` 已实现并合入本地 `main`，等待合入后的开发者验收；`T-049` 正在检修完整前端测试；Job/Workspace UI 仍分别等待 `DEC-004`/`DEC-005`。
- 当前焦点：先收口 `T-049` 的测试环境/断言问题与 SessionList 拖拽失败定责，同时保留 `T-048` 的真人验收入口。
- 可执行：`T-049` 无决策阻塞；Job/Workspace UI 仍按各自决策局部阻塞。
- TA 执行中：`T-049`，等待派发结果。
...
> `T-049` 是当前执行中的测试检修；`T-048` 已完成实现并合入本地 `main`，当前等待开发者验收；其余条目为已实现或已合入、等待开发者验收或收口的近期交付。

### T-049：完整前端 Vitest 失败检修与归因

- 任务类型：代码改动（测试维护）｜阶段/状态：待派发 / 调查与修复｜合入 main：否｜开发者验收：待确认
...
- 阻塞：无。
- 下一动作：派发 TA 调查、修复并报告分类、差异、commit 与完整测试结果。
- [ ] 修复 Toast jsdom 测试环境

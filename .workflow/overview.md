...
- 整体状态：T-021 UI demo 根据开发者反馈进行第二轮修复，等待 TA 报告
- 当前焦点：T-021；继续只做前端 demo，后端仍等待开发者确认
- 可执行：等待 TA 修复文字重复、附件节点重拖动和直接上传 mock
...
- 跟进任务：`attachment-dnd-ui-demo-followup-20260913`，复用上述 TA/Worker 和分支；当前 follow-up 尚未提交。
- 有序待办：
  - [x] 实现拖动、插入光标和附件节点 UI
  - [x] 添加回归测试并完成定向验证
  - [x] 提供 demo 启动方法和已知限制

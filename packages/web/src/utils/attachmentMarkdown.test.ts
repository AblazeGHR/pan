...
describe('attachment Markdown protocol', () => {
  it('keeps the original display name while targeting the random storage name', () => {
    ...
    expect(attachmentMarkdown({
      ...
      href,
    })).toBe('[需求说明 \\[v1\\]\\(最终\\).md](/api/attachments/upload_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.md?session_id=s1)');
  });

  it('URL-encodes path characters and rejects dangerous destinations', () => {
    const href = serverFileDownloadHref('s1', 'D:\\files\\a [b](c)#.md');
    expect(href).toBe('/api/fs/read?session_id=s1&path=D%3A%5Cfiles%5Ca+%5Bb%5D%28c%29%23.md&download=1');
    expect(isSafeAttachmentHref(href)).toBe(true);
    expect(isSafeAttachmentHref('https://evil.example/upload_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.md')).toBe(false);
    expect(isSafeAttachmentHref('/api/attachments/../etc/passwd?session_id=s1')).toBe(false);
    expect(isSafeAttachmentHref('/api/attachments/upload_aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa.md')).toBe(false);
  });

...
export function InputRow() {
  ...
  const queueCount = useQueueStore((s) => {
    ...
    const server = s.agentQueues[currentSessionId];
    return (q ? q.length : 0) + (e ? 1 : 0) + (server ? server.length : 0);
  });
  ...
  const handleSend = useCallback(
    async (text: string) => {
      ...
      // Every normal user send goes through the durable server queue.  This is
      // intentionally independent of workerStatus: a running worker keeps the
      // current turn, while this message is appended behind it in FIFO order.
      try {
        const clientMessageId = typeof crypto !== 'undefined' && crypto.randomUUID
          ? crypto.randomUUID()
          : `${Date.now()}-${Math.random().toString(16).slice(2)}`;
        await sendSession(currentSessionId, text, clientMessageId);
        addMessage({ role: 'user', content: text });
        if (inputRef.current) inputRef.current.value = '';
        setInputDraft(currentSessionId, '');
        void useQueueStore.getState().loadAgentQueue(currentSessionId);
      } catch (e) {
        showToast(
